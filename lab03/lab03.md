...
2. Если же узким местом является вторая линия связи ($R_C < R_S$), то второй пакет может находиться во входном буфере перед второй линией, ожидая своей передачи.\
   Посчитаем:
    ...
    1. Первому пакету, чтобы полностью прийти (дойти и выйти) к маршрутизатору, требуется:
    $$$t_1 := \frac{L}{R_S} + d_\text{распространения} + \frac{L}{R_C} $$$
    2. Второму пакету, чтобы дойти до маршрутизатора, требуется:
    $$$t_2 := \frac{L}{R_S} + T + \frac{L}{R_S} + d_\text{распространения} $$$
    3. Чтобы второй пакет не попал в очередь во вторую линию связи, должно выполняться следующее неравенство:
    $$$
    \begin{align}
       t_1 &\leq t_2 \\
...
#### Ответ: $ T_\text{мин} = \frac{L}{R_C} - \frac{L}{R_S} $
...
#### Решение
1. Среднее время передачи объекта по каналу связи: $\Delta = \dfrac{\text{средний размер объекта}}{\text{средняя скорость канала}} = \dfrac{85 \cdot 10^4\text{ бит}}{15 \cdot 10^6 \text{ бит/с}} \approx 0.05667 \text{ с} = 56.67 \text{ мс}$
2. 1. Для нахождения средней задержки используем формулу из условия: $$$\dfrac{\Delta}{1-\Delta\cdot B}= \dfrac{0.05667}{1-0.05667 \cdot16} = 0.607525729 \text{ с}$$$
...
3. 1. Интенсивность трафика на канале снижается на $1-0.4 = 0.6 \space (60\%)$\
Значит средняя задержка становится равной: $$$\dfrac{\Delta}{1-\Delta\cdot (0.4\cdot B)}= \dfrac{0.05667}{1-0.05667 \cdot (0.4 \cdot 16)} \approx 0.08892 \text{ с}$$$
