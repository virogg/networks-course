...
1. Запрос первого объекта (TCP-рукопожатие + запрос + ответ):<br>
```math
\underbrace{3\cdot(T + d_{\text{распр}})}_{\text{SYN + SYN-ACK + GET}} + (d_{\text{пер}} + d_{\text{распр}}) \approx 670.657 \text{ c}
```

2. $10$ соединений, каждое получает $R/10 = 15$ бит/с.<br>
  ```math
\underbrace{3\cdot(T\cdot 10 + d_{\text{распр}})}_{\text{SYN+SYN-ACK+GET}} + (d_{\text{пер}}\cdot 10 + d_{\text{распр}}) \approx 6706.57 \text{ c}
```
4. Итого: $6706.57 + 670.657 = 7377.2$ с

#### 2. постоянных HTTP-соединений
1. Запрос первого объека остается точно такой же:<br>
```math
3\cdot(T + d_{\text{распр}}) + (d_{\text{пер}} + d_{\text{распр}}) \approx 670.657 \text{ с}
```
2. 10 объектов последовательно (TCP уже установлен):<br>
```math
10\cdot(T + d_{\text{распр}} + d_{\text{пер}} + d_{\text{распр}}) \approx 6679.97 \text{ c}
```
3. Итого: $670.657 + 6679.97 = 7350.627$ с
